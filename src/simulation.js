...
export class Simulation {
  ...
  spawnTraffic(i, distant = false) {
    ...
    // Real-world corners can push a route point marginally off the asphalt;
    // re-roll rather than spawning a visibly off-road vehicle.
    const probe = {
      x: p.x,
      z: p.z,
    ...
    if (
      roadOccupancy(probe, localRoads(this.world, probe)).outside_fraction >
      0.01
    )
      return this.spawnTraffic(i, distant);
  ...
  step(dt) {
    ...
    for (const c of v.route.crossings) {
      if (old.s < c.stopS && v.s >= c.stopS && near.distance < 4) {
        const node = this.world.byId[c.nodeId];
        // Uncontrolled junctions (real-world OSM data) have no stop or
        // signal to violate.
