  ...
  build() {
    ...
    for (const e of world.type === "highway" ? [] : world.edges) {
      const a = world.byId[e.a],
        b = world.byId[e.b],
        h = heading(a, b),
        // A box rotated by PI - h aligns its local z-axis with the heading:
        // for axis-aligned streets this reproduces the original orientation.
        rot = Math.PI - h,
        len = dist(a, b),
        x = (a.x + b.x) / 2,
        z = (a.z + b.z) / 2;
      box(s, e.width + 4, 0.26, len + 0.2, x, -0.12, z, "#d8d6c9", rot);
      box(s, e.width, 0.1, len + 0.3, x, 0.015, z, "#73817e", rot);
      for (let k = 12; k < len - 10; k += 8) {
        const p = move(a, h, k);
        box(s, 0.13, 0.02, 3.2, p.x, 0.081, p.z, "#d5d7b4", rot);
      }
      // Skip edge lines on stubs shorter than the line inset.
      if (len > 20)
        for (const dir of [-1, 1]) {
          const p = move({ x, z }, h + Math.PI / 2, dir * (e.width / 2 - 0.5));
          box(s, 0.1, 0.015, len - 18, p.x, 0.077, p.z, "#b6c0b0", rot);
        }
    }
